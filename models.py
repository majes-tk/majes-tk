...
class Order(db.Model):  # type: ignore
    ...
    price = db.Column(db.Float, unique=False, default=0)
...
class OrderReply(db.Model):  # type: ignore
    id = db.Column(db.Integer, primary_key=True)
...
class Restaurant(db.Model):  # type: ignore
    ...
    tally_amount = db.Column(db.Float, default=0, nullable=False)
    delivery_cost = db.Column(db.Float, default=0, nullable=True)
    ...
    def __repr__(self):
        return "<Restaurant %s of user %s>" % (self.name, self.linked_user.username)


class Dish(db.Model):  # type: ignore
    id = db.Column(db.Integer, primary_key=True)
...
class OrderItem(db.Model):  # type: ignore
    ...
    order = db.relationship(
        'Order', backref="Order_of", foreign_keys=[order_id])
    dish = db.relationship(
        'Dish', backref="Order_with", foreign_keys=[dish_id])

    def __repr__(self):
        return "<OrderItem %s in user Order #%s>" % (self.dish, self.order.id)
