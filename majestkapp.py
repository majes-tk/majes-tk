...
def createTicket():
    if "login" in session.keys() and session['login']:
        if request.method == 'POST':
            user.create_ticket(
                request.form["ticket-title"], request.form["ticket-text"], None, g.current_user, None)
    ...
    if "login" in session.keys() and session['login']:
        if request.method == 'POST':
            if request.form.get('action') == "SaveNote":
                user.create_ticket_reply(
                    request.form["reply-text"], None, g.current_user, ticketid, isNote=True)
            else:
                user.create_ticket_reply(
                    request.form["reply-text"], None, g.current_user, ticketid)
            return redirect(url_for('viewTicket', ticketid=ticketid))
        return redirect(url_for('viewTicket', ticketid=ticketid))
    else:
        abort(403)
...
def addUser():
    if request.method == 'POST':
        try:
            user.create_user(str.lower(request.form["username"]), request.form["email"], user.hashPassword(
                request.form["password"]), highPermissionLevel=False)
        except sqlalchemy.exc.IntegrityError:
